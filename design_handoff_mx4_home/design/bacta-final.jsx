...
/* Command dock — Ask MX-4 (wears the room's accent) + hex nav launcher */
function BactaDock({ accent = MX4, onAsk, onNav }) {
  const { color } = BACTA;
  return (
    <div style={{ position: 'relative', zIndex: 1, background: 'rgba(17,24,39,0.92)', borderTop: `1px solid ${hexA(accent,0.28)}`, padding: '10px 18px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
      <button onClick={onAsk} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'transparent', border: 'none', padding: '4px 4px', cursor: 'pointer' }}>
        <span style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', width: 44, height: 44, borderRadius: '50%', background: `radial-gradient(circle, ${hexA(accent,0.13)}, ${hexA(accent,0.03)} 70%)`, border: `1px solid ${hexA(accent,0.55)}`, animation: 'mx4glowbreathe 3.6s ease-in-out infinite', flexShrink: 0 }}>
          <MX4Sigil color={accent} size={28} glow mood="listen" />
        </span>
        <span style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 2 }}>
          <span style={{ fontFamily: BACTA.FONT_UI, fontSize: 13.5, fontWeight: 650, color: color.text, letterSpacing: '-0.01em', whiteSpace: 'nowrap' }}>Ask MX-4</span>
          <span style={{ fontFamily: BACTA.FONT_MONO, fontSize: 8.5, letterSpacing: '0.12em', color: color.text3, whiteSpace: 'nowrap' }}>TAP TO TALK</span>
        </span>
      </button>
      <button onClick={onNav} aria-label="All systems" style={{ flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', width: 44, height: 44, borderRadius: '50%', border: `1px solid ${color.line}`, background: color.base, cursor: 'pointer', padding: 0 }}>
        <NavIcon color={color.text2} size={26} />
      </button>
    </div>
  );
}

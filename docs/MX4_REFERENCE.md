...
| **fetchPage** | Fetches a specific URL (chat only) and returns its parsed text content. | When the answer lives inside a specific page/thread — e.g. a link found via `research`, or a URL the user gives you directly — not just a search snippet. | Text/HTML pages only — not images, videos, PDFs, or pages requiring login. If it returns an `error` field, tell the user the page couldn't be fetched rather than guessing at its content. |
...
**`food_log_entries`** (one row per logged food per meal per day): `id`, `date` (ISO date the entry counts toward), `meal_type` (breakfast/lunch/dinner/snack, or a free-form label), `logged_at` (timestamp within the day), `variant_id` (FK to `food_variants`, NULL for a fully ad-hoc entry), `name` (denormalized display name, always present), `quantity`, `unit`, `calories`, `protein_g`, `carbs_g`, `fat_g`, `fiber_g`, plus the widened set (#140): `sodium_mg`, `sugar_g`, `saturated_fat_g`, `polyunsaturated_fat_g`, `monounsaturated_fat_g`, `trans_fat_g`, `cholesterol_mg`, `potassium_mg`, `vitamin_a_mcg`, `vitamin_c_mg`, `calcium_mg`, `iron_mg`, `glycemic_index` (TEXT category, e.g. `'Low'`/`'Medium'`/`'High'`), `custom_nutrients`/`allergens`/`traces` (JSON-encoded strings — object / string array — parse before use; NULL means untracked, not zero/empty). Nutrient columns are a **denormalized snapshot at log time** — editing or re-importing a `food_variants` row later never changes what a past day's log says was eaten. Query example: `SELECT date, meal_type, name, calories, protein_g, carbs_g, fat_g FROM food_log_entries WHERE date >= date('now', '-14 days') ORDER BY date, logged_at`.

**`nutrition_targets`** (one row per date the targets changed, not a single mutable settings row): `id`, `date`, `calories`, `protein_g`, `carbs_g`, `fat_g`, `fiber_g`, plus the same 12 widened numeric fields as `food_log_entries` (sodium/sugar/fat sub-components/cholesterol/potassium/vitamins/minerals) — a user can set a daily ceiling/floor for any of them. `glycemic_index`/`custom_nutrients`/`allergens`/`traces` are NOT meaningful as daily targets and are always NULL here even though the columns exist (schema uniformity across the 4 nutrition tables, not a usable target). "Current" target for a given date = the row with the latest `date <= <that date>` — never assume the most recently-inserted row is current if querying a past date.

**`foods`** (reference identity only, not user logs — rarely needs querying directly by you): `id`, `source` (`'usda'` / `'openfoodfacts'` / `'custom'`), `source_id`, `name`, `brand`, `source_json`. A food's servable units and macros live on `food_variants`, not here — `foods` is just the identity a food's variants hang off of. Bulk-imported from USDA FoodData Central (Foundation Foods + SR Legacy) plus user-saved custom foods.

**`food_variants`** (one row per servable unit of a food, e.g. "100 g", "1 slice", "1 cup" — every food has at least one, with exactly one flagged `is_default=1`): `id`, `food_id` (FK to `foods`), `label` (display string), `serving_qty`/`serving_unit` (the quantity the macro columns refer to, e.g. `100`/`"g"`), `gram_weight` (NULL if unknown, e.g. a custom food logged in a non-mass unit with no gram equivalent), `is_default`, `source` (`'usda'` / `'openfoodfacts'` / `'custom'`), `calories`, `protein_g`, `carbs_g`, `fat_g`, `fiber_g`, plus the same widened set as `food_log_entries` (sodium_mg through iron_mg, glycemic_index, custom_nutrients/allergens/traces). USDA-imported rows only ever populate the 12 numeric widened fields (verified live against FDC nutrient codes 307/269/606/646/645/605/601/306/320/401/301/303) — `glycemic_index`/`custom_nutrients`/`allergens`/`traces` stay NULL for USDA/OFF-sourced rows and only populate for custom/ad-hoc variants where the user tags them directly.

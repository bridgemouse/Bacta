...
**`foods`** (reference/ingredient data, not user logs — rarely needs querying directly by you): `id`, `source` (`'usda'` / `'openfoodfacts'` / `'custom'`), `source_id`, `name`, `brand`, `default_qty`/`default_unit` (the quantity the macro columns refer to), `calories`, `protein_g`, `carbs_g`, `fat_g`, `fiber_g`, plus the same widened set as `food_log_entries` (sodium_mg through iron_mg, glycemic_index, custom_nutrients/allergens/traces), `source_json`. USDA-imported rows only ever populate the 12 numeric widened fields (verified live against FDC nutrient codes 307/269/606/646/645/605/601/306/320/401/301/303) — `glycemic_index`/`custom_nutrients`/`allergens`/`traces` stay NULL for USDA/OFF-sourced rows and only populate for custom/ad-hoc foods where the user tags them directly. Empty except for user-saved custom foods until a bulk import ships (not yet built).

...
function BactaApp() {
  ...
  const accent = isHome ? MX4 : BACTA.section[view].accent;
  ...
  return (
    <div style={{ position: 'relative', height: '100%', display: 'flex', flexDirection: 'column', background: color.base, fontFamily: BACTA.FONT_UI, color: color.text, overflow: 'hidden' }}>
      ...
      {isHome
        ...
        : <BactaStatusBar accent={accent} title={BACTA.section[view].label.toUpperCase()} sectionKey={view} onBack={() => setView('home')} />}

      {isHome
        ? <HomeBody onOpenSection={setView} />
        : <SectionShell sectionKey={view} />}

      <BactaDock accent={accent} onAsk={() => setAsk(true)} onNav={() => setNav(true)} />

      <NavSheet open={nav} onClose={() => setNav(false)} current={view} onNavigate={(k) => { setView(k); setNav(false); }} />

...
**Backend (Phases 1–6): implemented, awaiting review.** Branch `feature/nutrition-backend` (worktree `worktrees/nutrition-backend`) implements a complete, usable backend: schema (`foods`, `food_log_entries`, `nutrition_targets`; `macrofactor_snapshots` dropped — confirmed empty, superseded by these purpose-built tables since EAV can't represent multi-item-per-day logging), food search + custom-food save, food-log CRUD (server-side macro scaling from `foods`), targets + summary, a zero-filled trend endpoint, and MX-4 orchestrator integration (`nutrition` added to `SECTIONS`, `queryDb` schema description, `MX4_REFERENCE.md` Data Dictionary). TDD throughout; full test suite (360 server + 279 client) and both typechecks green. Not yet merged.  
**Still open:** Phases 7–8 (bulk USDA FoodData Central / Open Food Facts import) — deferred, not a prerequisite for the backend above to be useful (FR3/FR8's ad-hoc-entry-and-save loop covers the common single-user case); blocked on a human downloading multi-GB dump files and on `NUTRITION_PLAN.md` §7's open question of TypeScript vs. Python for the import script. **No frontend** — `NutritionPage.tsx` stays a stub and `nutrition` stays out of `client/src/theme.ts`'s `BUILT_SECTIONS` until a Claude Design session produces UI from `docs/NUTRITION_DESIGN_BRIEF.md`. Other open product decisions in `NUTRITION_PLAN.md` §7: whether `home`'s cross-channel synthesis should include nutrition (sequenced so this can be answered "yes" later without a re-order), meal-type vocabulary strictness, unit conversion, and micronutrient surfacing.

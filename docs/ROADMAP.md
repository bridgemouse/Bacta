...
**Backend (Phases 1–8): implemented, awaiting review.** Branch `feature/nutrition-backend` (worktree `worktrees/nutrition-backend`, PR #136) implements the full backend, all 8 phases: schema (`foods`, `food_log_entries`, `nutrition_targets`; `macrofactor_snapshots` dropped — confirmed empty, superseded by these purpose-built tables since EAV can't represent multi-item-per-day logging), food search + custom-food save, food-log CRUD (server-side macro scaling from `foods`), targets + summary, a zero-filled trend endpoint, MX-4 orchestrator integration (`nutrition` added to `SECTIONS`, `queryDb` schema description, `MX4_REFERENCE.md` Data Dictionary), and USDA FoodData Central / Open Food Facts bulk-import mapping + a batch loader CLI (`scripts/nutrition/importFoods.ts --usda <path> --off <path>`). TDD throughout, including two rounds of fresh-subagent audit that each caught and fixed real bugs before commit. Full test suite (376 server + 279 client) and both typechecks green. Not yet merged.  
**Bulk import — USDA staged, Open Food Facts deliberately skipped.** The mapping/loader code (Phases 7–8) is real and tested against fixtures built from live USDA/OFF API records (not guessed field names). USDA FoodData Central's Foundation Foods + SR Legacy JSON files (213 MB total) have been downloaded to LXC 109 at `/opt/bacta/data/nutrition-import/` and are ready to import. **Open Food Facts' full global export was checked and rejected**: the JSONL dump is 12.46 GB compressed (confirmed via a live HEAD request), likely 100+ GB uncompressed — LXC 109 only has a 20 GB disk, and the current loader isn't a streaming reader, so it can't process a file that size regardless of disk space. Decision: don't pursue OFF for now (matches the plan's own original scope-discipline judgment call, §7 open question 2) rather than resize the container and rewrite the loader for streaming just to cover packaged/branded foods USDA already doesn't need. The OFF code path stays built and tested but won't be exercised.  
**Do not run the USDA import against production until this PR merges.** Confirmed live via the `bacta-sqlite` MCP: the running production DB (main branch, unmerged) still has the old `macrofactor_snapshots` table and none of `foods`/`food_log_entries`/`nutrition_targets` — the schema migration hasn't been applied yet. Once merged (and `bacta-api` restarts and runs `migrate()`), run `npx tsx scripts/nutrition/importFoods.ts --usda /opt/bacta/data/nutrition-import/FoodData_Central_foundation_food_json_2026-04-30.json` (and the SR Legacy file, same flag) against the real DB. Until then, `foods` stays populated only by whatever the user has saved via the app itself (FR8) — see `docs/NUTRITION_DESIGN_BRIEF.md`'s revision for why this matters for how the frontend should be designed.  
**No frontend** — `NutritionPage.tsx` stays a stub and `nutrition` stays out of `client/src/theme.ts`'s `BUILT_SECTIONS` until a Claude Design session produces UI from `docs/NUTRITION_DESIGN_BRIEF.md`. Other open product decisions in `NUTRITION_PLAN.md` §7: whether `home`'s cross-channel synthesis should include nutrition (sequenced so this can be answered "yes" later without a re-order), meal-type vocabulary strictness, unit conversion, and micronutrient surfacing.

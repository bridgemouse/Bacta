export const METRICS: Record<string, { unit: string; description: string }> = {
  ...
  sleep_s:              { unit: 's',          description: 'Total sleep duration' },
  sleep_deep_s:         { unit: 's',          description: 'Deep sleep duration' },
  sleep_rem_s:          { unit: 's',          description: 'REM sleep duration' },
  sleep_light_s:        { unit: 's',          description: 'Light sleep duration' },
  spo2_avg:             { unit: '%',          description: 'Blood oxygen saturation' },
  resp_avg:             { unit: 'brpm',       description: 'Breathing rate' },
...
export const METRIC_SOURCES: Record<string, Record<string, string>> = {
  garmin: {
    ...
    spo2_avg:     'optical sensor',
    stress:       'HRV-derived',
    resp_avg:     'optical sensor',
    steps:        'accelerometer',

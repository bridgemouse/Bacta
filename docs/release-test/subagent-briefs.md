...
- **No PHI in git — CONFIRMED ISSUE (critical):** `mx4/wiki/` is in `.gitignore` (line 11) **but the files are still tracked** (`git ls-files mx4/wiki/` lists `ethan-profile.md`, `hrv-patterns.md`, `weekly-observations.md`, etc.) — `.gitignore` doesn't untrack already-committed files, so personal health data is in the working tree **and in git history, likely already on GitHub origin**. Remediate deliberately (outward-facing — coordinate/gate): `git rm --cached -r mx4/wiki/` to untrack, then scrub history (`git filter-repo` or BFG), force-push, and treat any exposed content as compromised. Also scan the rest of the repo + history for committed health data, vault content, or `bacta.db`; confirm `data/*.db` and any `*.bak-*` are ignored.

...
2. **Multi-device integration — DONE (2026-06-28, PR #6 merged to main).** Plans 1–3 complete: table migrations, 6-provider OAuth/API-key integration layer, Settings UI (CONNECTED DEVICES + DATA PRIORITY rails), source badges on Recovery + Sleep, Garmin as full peer. 445 tests. See below for full detail. **Caveat found in the 2026-07-07 codebase audit:** "DONE" covered the integration layer and manual-sync UI, not full automation or data correctness. Connected providers other than Garmin had no background sync at all — the dead `scripts/health_poller.py` dispatcher this was meant to run through was never wired to any timer/cron (issue #81, PR pending review: an in-process hourly scheduler in `server/lib/providerSync.ts` now calls `runSync()` directly for each enabled provider, and `health_poller.py` was removed). Oura/Whoop/Polar/Withings also wrote sleep/SpO2/respiration metrics under names the client never reads, making that data permanently invisible (issue #83, PR pending review: providers now write under the Garmin-derived metric vocabulary).

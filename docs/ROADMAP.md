...
> `POST /api/poll/force` and the MX-4 nightly cron were both previously listed here as
> unverified. `check_signal.py` no longer exists — the poll-force endpoint runs the
> poller directly via `spawn('python3', [garmin_poller.py])` (`server/api/poll.ts:10-12`)
> — and `app_logs` confirms the nightly orchestrator run has completed reliably every
> day since at least 2026-07-29. Both moved off this list.
...
2. **Multi-device integration — DONE (2026-06-28, PR #6 merged to main).** Plans 1–3 complete: table migrations, 6-provider OAuth/API-key integration layer, Settings UI (CONNECTED DEVICES + DATA PRIORITY rails), source badges on Recovery + Sleep, Garmin as full peer. 445 tests. See below for full detail. **Caveat found in the 2026-07-07 codebase audit, closed 2026-07-09:** "DONE" covered the integration layer and manual-sync UI, not full automation or data correctness. Connected providers other than Garmin had no background sync at all — the dead `scripts/health_poller.py` dispatcher this was meant to run through was never wired to any timer/cron (issue #81, PR #106: an in-process hourly scheduler in `server/lib/providerSync.ts` now calls `runSync()` directly for each enabled provider, and `health_poller.py` was removed; review caught and fixed a missing in-flight guard — a hung provider API call, with no fetch timeout anywhere in the integration layer, could let the next cron tick start an overlapping second sync pass). Oura/Whoop/Polar/Withings also wrote sleep/SpO2/respiration metrics under names the client never reads, making that data permanently invisible (issue #83, PR #104: providers now write under the Garmin-derived metric vocabulary).
...
5. **LogEntry Phase C — DONE.** The activity log expand panel (Training Effect bars, HR zone distribution, Running Dynamics grid, multi-sport per-leg breakdown) is implemented in `LogEntry.tsx`; `hasContent` is computed dynamically from whichever fields the activity actually has, not gated behind a static flag.
...
8. **Silent-failure sweep — DONE (2026-07-08, issues #63, #65–#72 from the 2026-07-07 codebase audit, PRs #86–#94).** Reviewed and merged to `main`. Closes a cluster of "caught but never logged" gaps found in the audit: orchestrator + chat vault-tool failures, poll/force-sync and provider-sync and OAuth-callback failures, chat compression failures, sleep-hypnogram parse/shape failures, and nightly-cron failures now all write to `app_logs` instead of `console.error`-only or silent swallowing; `/api/garmin/activities?days=` also got a missing lower-bound clamp (`days=-5` no longer silently returns empty). Each PR is TDD (RED commit shows the pre-fix failing test) with full suite + typecheck green.
9. **Remaining audit backlog cleared — DONE (2026-07-09, issues #73–79, #81–85 from the 2026-07-07 audit, PRs #95–#108).** Two `bacta-headless` runs produced all 14 PRs; a full `/bacta-pr-review` session reviewed and merged every one to `main` same-day. 12 merged clean; review found and fixed real bugs in 2: **#99** (`useSyncState.ts`) — retrying a sync within the 3s post-error idle-reset window left the old timer armed, so it could silently reset an in-flight retry back to idle; **#106** (`server/lib/providerSync.ts`) — see item 2 above for the in-flight-guard fix, plus 4 stale poller.py docstrings (oura/polar/whoop/withings) that PR's own deletion of `health_poller.py` would have orphaned, fixed in the same pass. Also closed: issue #37 (MX-4 briefing pop-in on first section visit — root-caused via real iOS device testing to a data-loading layout jump, fixed with `prefetchBriefing()` cache-warming from Home + a `valueFadeIn` fallback for Home's own first load, PR #103) and issue #82 (second stale duplicate Garmin script found — `scripts/providers/garmin/poller.py` — removed; a related duplicate, `scripts/providers/garmin/ingest.py`, was found during that review and filed separately as issue #109). Main: 312 server + 272 client tests, 0 open PRs.
...
**Status: COMPLETE and LIVE.** Custom-built nutrition tracker — not a third-party integration. Backend, frontend, and the USDA bulk import are all merged and running in production.

**Backend (Phases 1–8):** merged via PR #136. Schema (`foods`, `food_log_entries`, `nutrition_targets`; `macrofactor_snapshots` dropped — confirmed empty, superseded by these purpose-built tables since EAV can't represent multi-item-per-day logging), food search + custom-food save, food-log CRUD, targets + summary, a zero-filled trend endpoint, MX-4 orchestrator integration (`nutrition` in `SECTIONS`, `queryDb` schema description, `MX4_REFERENCE.md` Data Dictionary), and USDA FoodData Central / Open Food Facts bulk-import mapping + loader CLI (`scripts/nutrition/importFoods.ts --usda <path> --off <path>`).

**Frontend:** merged via PR #138 (Jul 15, 2026) — full 17-task build (Overview, Log Entry sheet, Edit Entry sheet, Targets sheet, Library with food/recipe creation). `nutrition` is in `client/src/theme.ts`'s `BUILT_SECTIONS`. Multiple headless-backlog rounds (Jul 17–19, PRs #149–158) landed perf and correctness fixes on top (debounced search, parallelized copy-meal, live-rescale bugs, etc.).

**Multi-serving model (`food_variants`):** merged via PR #169 (Jul 28, 2026), 13-task plan at `docs/superpowers/plans/2026-07-20-nutrition-food-variants-plan.md`. Replaces the old "one food = one locked unit" shape — `foods` is now identity-only, `food_variants` holds one row per servable unit (each with its own precomputed macros), and `food_log_entries`/`recipe_ingredients` reference `variant_id` with `quantity` meaning "count of that variant's serving." A follow-up display bug (meal-entry line misreading servings-count as a raw amount) was found via live Playwright verification and fixed same-day (commit `9407d1e`). Independently re-verified Jul 29: live DB has 8,157 foods / 22,972 variants (94.5% of foods have 1–4 variants, matching the design spec's projection), a real logged entry's stored macros check out exactly against its variant's per-serving values, and the full suite (902 tests) + both typechecks are clean. One stale test found during that audit (`tests/server/nutrition.test.ts`, still posting pre-migration `food_id`/`default_qty` fields) fixed in PR #170.

**Bulk import — USDA live, Open Food Facts deliberately skipped.** The USDA FoodData Central import (Foundation Foods + SR Legacy) has been run against the real production DB — this is no longer pending. **Open Food Facts' full global export was checked and rejected**: the JSONL dump is 12.46 GB compressed, likely 100+ GB uncompressed — LXC 109 only has a 20 GB disk and the loader isn't a streaming reader. The OFF code path stays built and tested but unexercised; revisit only if branded/packaged foods become a real gap.

**Open product decisions (`NUTRITION_PLAN.md` §7):** whether `home`'s cross-channel synthesis should include nutrition, meal-type vocabulary strictness, unit conversion, and micronutrient surfacing. None of these block current functionality.
...
**`MX4Card` stub — already removed.** `client/src/components/MX4Card.tsx` only exports `MX4Briefing` and `TransmissionPanel`; the null-returning `MX4Card` stub is gone (see "Tech debt cleared" above).

...
**Bulk import is built but not run.** The mapping/loader code is real and tested against fixtures built from live USDA/OFF API records (not guessed field names) — but it has never been run against the actual multi-GB USDA Foundation Foods / SR Legacy or Open Food Facts export files, because downloading them to the host is a manual, human step outside what this session could do. Someone needs to: download the files (`fdc.nal.usda.gov/download-datasets`, `world.openfoodfacts.org/data`) to LXC 109, then run `npx tsx scripts/nutrition/importFoods.ts --usda <path> --off <path>` against the real production DB. Until that happens, `foods` stays populated only by whatever the user has saved via the app itself (FR8) — see `docs/NUTRITION_DESIGN_BRIEF.md`'s revision for why this matters for how the frontend should be designed.  

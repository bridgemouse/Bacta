...
const ZONES_INFO: CardInfo = {
  ...
  content: () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4, width: '100%' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 3, borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: 5 }}>
        {([
          { zone: 'Z1', range: '<60%',    label: 'EASY',      color: COLORS.textMuted,          note: 'Recovery. Preserves readiness, no extra stress.' },
          { zone: 'Z2', range: '60–70%',  label: 'AEROBIC',   color: A,                          note: 'Aerobic base. Mitochondria, fat oxidation. Most valuable.' },
          { zone: 'Z3', range: '70–80%',  label: 'TEMPO',     color: COLORS.amber,              note: 'Crossover zone. Useful gains, accumulates fatigue.' },
          { zone: 'Z4', range: '80–90%',  label: 'THRESHOLD', color: hexA(COLORS.mx4Red, 0.85), note: 'Lactate threshold. Raises your ceiling fast.' },
          { zone: 'Z5', range: '90–100%', label: 'VO₂ MAX',   color: COLORS.mx4Red,             note: 'Peak output. Max cardio gains. Brief intervals only.' },
          ...
          <div key={zone} style={{ display: 'flex', alignItems: 'baseline', gap: 5 }}>
            ...
            <span style={{ fontFamily: FONT_UI, fontSize: 9, color: 'rgba(255,255,255,0.72)', lineHeight: 1.2 }}>{note}</span>
          </div>
        ))}
      </div>
      <p style={{ margin: 0, fontFamily: FONT_UI, fontSize: 9, fontStyle: 'italic', color: 'rgba(255,255,255,0.48)', textAlign: 'center', lineHeight: 1.3 }}>
        80/20 rule — ~80% Z1–2 and ~20% Z3–5 each week for sustainable gains.
      </p>
    </div>
  ),
}
